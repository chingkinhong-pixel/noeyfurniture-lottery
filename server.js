...
const userSchema = new mongoose.Schema({
    ...
    status: { type: String, default: 'active' } // active, invalid
});
...
const deleteLogSchema = new mongoose.Schema({
    ...
    phone: { type: String, required: true },
    reason: { type: String, default: "系统清理" },
...
async function initData() {
    try {
        ...
        // 启动时自动清洗：将过去非法手机号打上 invalid 标签
        const allUsers = await User.find({ role: 'user' });
        const phoneRegex = /^1[3-9]\d{9}$/;
        for (let u of allUsers) {
            if (!phoneRegex.test(u.phone) && u.status !== 'invalid') {
...
app.get('/api/admin/dashboard', requireAdmin, async (req, res) => {
    try {
        ...
        const funnel = { '未联系': 0, '已咨询': 0, '已量房': 0, '已报价': 0, '已成交': 0, '已完成安装': 0 };
        customers.forEach(c => {
            const status = c.followUpStatus || '未联系';
    ...
    try {
        ...
        for (let phone of phones) {
            ...
            await DeleteLog.create({
                userId: userId,
                phone: user.phone,
                reason: reason || '后台主动清理',
                operator: req.adminPhone
            });

            // 2. 使用底层身份标识级联清理周边所有的业务孤岛数据
            // 注：由于原结构强依赖 phone 作为外键，这里统一通过 phone 精准对齐删除
            await Customer.deleteOne({ phone });
            await RewardRecord.deleteMany({ phone });

            // 3. 销毁用户主体资料（同时会拔除包裹在其内的分享数据 shareStatus/invitees）
            await User.deleteOne({ _id: userId });
            
            deletedCount++;
        }
        
        // 删除后由于是真实销毁，下次 dashboard 请求将自动计算新汇总值
        res.json({ success: true, deletedCount });
    } catch (e) {
        console.error(e);
        res.status(500).json({ error: '级联删除失败' });
    }
});
